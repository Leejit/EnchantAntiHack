			...
			if($i instanceof Item){
				if($i->hasEnchantments()){
					foreach($i->getEnchantments() as $e){
						if($e->getLevel() >= $max){
                             if ($p->hasPermission("keep.op.item")) return true;
							$p->getInventory()->removeItem($i);
							$this->getServer()->getLogger()->info(TF::GREEN."".TF::BLUE."Item ".$i->getName()." has been removed from ".$p->getName()."'s inventory for a enchantment level over ".$this->getMax()."!");
							$p->sendMessage(TF::GREEN."".TF::BLUE.$i->getName()." has been removed from your inventory for being above or equal to the max enchantment level!");
						}
					}
				}
			}
